...
class PHPLogin
{
    ...
    public function editUserName($user_name)
    {
        ...
        // username cannot be empty and must be azAZ09 and 2-64 characters
            ...
            if (isset($result_row->user_id)) {
                ...
                $query_edit_user_name_tc = $this->db_connection->prepare('UPDATE account SET username = :user_name WHERE id = :user_id'); 
                $query_edit_user_name_tc->bindValue(':user_name', $user_name, PDO::PARAM_STR);
                $query_edit_user_name_tc->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
                $query_edit_user_name_tc->execute();

                if ($query_edit_user_name->rowCount() && $query_edit_user_name_tc->rowCount()) {
    ...
    public function editUserEmail($user_email)
    {
        ...
        // user mail cannot be empty and must be in email format
            ...
            if (isset($result_row->user_id)) {
                ...
                $query_edit_user_email_tc = $this->db_connection->prepare('UPDATE account SET email = :user_email WHERE id = :user_id');
                $query_edit_user_email_tc->bindValue(':user_email', $user_email, PDO::PARAM_STR);
                $query_edit_user_email_tc->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
                $query_edit_user_email_tc->execute();

                if ($query_edit_user_email->rowCount() && $query_edit_user_email_tc->rowCount()) {
        ...
        // all the above tests are ok
            ...
            if (isset($result_row->user_password_hash)) {
                ...
                if (password_verify($user_password_old, $result_row->user_password_hash)) {
                    ...
                    $user_password_hash_tc = SHA1(strtoupper($user_name.':'.$user_password_new));
                    ...
                    $query_update_tc = $this->db_connection->prepare('UPDATE account SET sha_pass_hash = :user_password_hash WHERE id = :user_id');
                    $query_update_tc->bindValue(':user_password_hash', $user_password_hash_tc, PDO::PARAM_STR);
                    $query_update_tc->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
                    $query_update_tc->execute();

                    // check if exactly one row was successfully changed:
                    if ($query_update->rowCount() && $query_update_tc->rowCount()) {
    ...
    public function getGravatarImageUrl($s = 50, $d = 'mm', $r = 'g')
    {
        if ($_SESSION['user_email'] != '') {
            ...
            return 'https://www.gravatar.com/avatar/' . md5(strtolower(trim($_SESSION['user_email']))) . "?s=$s&d=$d&r=$r&f=y";
        } else {
            return '';
        }
    }
    ...
    private function registerNewUser($user_name, $user_email, $user_password, $user_password_repeat, $captcha)
    {
        ...
        // finally if all the above checks are ok
            ...
            // Ok user can be create
                ...
                $user_password_hash_tc = SHA1(strtoupper($user_name.':'.$user_password));
                $query_new_user_insert_tc = $this->db_connection->prepare("INSERT INTO account (username, sha_pass_hash, email, last_ip, locked, expansion, os) VALUES (:user_name, :user_password_hash_tc, :user_email, '127.0.0.1', '1', '2', 'Win')");
                $query_new_user_insert_tc->bindValue(':user_name', $user_name, PDO::PARAM_STR);
                $query_new_user_insert_tc->bindValue(':user_password_hash_tc', $user_password_hash_tc, PDO::PARAM_STR);
                $query_new_user_insert_tc->bindValue(':user_email', $user_email, PDO::PARAM_STR);
                $query_new_user_insert_tc->execute();

                $user_id_tc = $this->db_connection->lastInsertId();
                ...
                if ($query_new_user_insert && $query_new_user_insert_tc) {
                    ...
                    if ($this->sendVerificationEmail($user_name, $user_id, $user_email, $user_activation_hash)) {
                        ...
                        $query_delete_user_tc = $this->db_connection->prepare('DELETE FROM account WHERE id=:user_id_tc');
                        $query_delete_user_tc->bindValue(':user_id_tc', $user_id_tc, PDO::PARAM_INT);
                        $query_delete_user_tc->execute();
    ...
    public function verifyNewUser($user_id, $user_activation_hash)
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $query_update_user_tc = $this->db_connection->prepare('UPDATE account SET locked = 0 WHERE id = :user_id');
            $query_update_user_tc->bindValue(':user_id', intval(trim($user_id)), PDO::PARAM_INT);
            $query_update_user_tc->execute();

            if ($query_update_user->rowCount() && $query_update_user_tc->rowCount()) {
